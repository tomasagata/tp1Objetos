...
class Stock extends Lista {
    ...
    actualizarCarritos(){
        /*
            El método manda a llamar a cada una
            de las instancias de carro de compras
            para que verifiquen que la cantidad en
            carrito <= cantidad en stock
        */

        this.carritos.forEach( (carrito) => {
            carrito.actualizarDatosCarrito();
        });
...
class Carrito extends Lista {
    ...
    quitarProducto(elemento, razon /* Opcional */ ) {
        this.listaProductos.forEach( (item) => {

            if(elemento != undefined && (item.prod.id == elemento.id || item.prod.id == elemento.prod.id)){
                this.listaProductos.splice(this.listaProductos.indexOf(item), 1);

                if(razon === undefined){
                    console.log("Se eliminó el elemento " + elemento.prod.nombreProducto + " del carrito de compras.");
                }
                else{
                    console.log("Se eliminó el elemento " + elemento.prod.nombreProducto + " del carrito de compras.\nRazón: " + razon );
                }
